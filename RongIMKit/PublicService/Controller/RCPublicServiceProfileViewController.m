...
- (NSArray *)cellCollections {
    if (!_cellCollections) {
        ...
        cell.onClickEvent = ^{
          if (self.serviceProfile.followed || self.serviceProfile.isGlobal) {
              [self enterPublicServiceConversation];
          } else {
              [self subscribePublicService];
          }
        };
...
- (void)subscribePublicService {
    ...
    RCProgressHUD *hud;
    hud = [RCProgressHUD showHUDAddedTo:self.tableView animated:YES];
    hud.labelText = NSLocalizedStringFromTable(@"Wait",@"RongCloudKit",nil);
    ...
    [[RCIMClient sharedRCIMClient] subscribePublicService:self.serviceProfile.publicServiceType publicServiceId:self.serviceProfile.publicServiceId success:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [RCProgressHUD hideHUDForView:weakSelf.tableView animated:YES];
        });
    } error:^(RCErrorCode status) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [RCProgressHUD hideHUDForView:weakSelf.tableView animated:YES];
